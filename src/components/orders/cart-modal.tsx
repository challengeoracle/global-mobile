...
import { CartItem } from "@/src/hooks/use-order-flow";
...
export function CartModal({ visible, items, totalAmount, syncing, message, onClose, onRemove, onQuantityChange, onCheckout }: Props) {
    const { colorScheme } = useColorScheme();
    const iconColor = colorScheme === "dark" ? "#f8fafc" : "#0f172a";

    return (
        <BottomSheetModal visible={visible} eyebrow="Pedido" title="QR para confirmação" onClose={onClose} maxHeightClassName="max-h-[85%]">
            <ScrollView showsVerticalScrollIndicator={false}>
                <View className="gap-3">
                    ...
                    <View className="mt-2 rounded-3xl border border-border bg-card p-4">
                        ...
                        <Text className="mt-3 text-sm leading-6 text-muted-foreground">Este botão não cria a venda sozinho. Ele gera um QR Code que o vendedor precisa escanear para confirmar.</Text>

                        <Pressable onPress={onCheckout} disabled={syncing || items.length === 0} className="mt-4 h-14 items-center justify-center rounded-2xl bg-primary disabled:opacity-50">
